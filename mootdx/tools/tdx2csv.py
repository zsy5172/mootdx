...
def txt2csv(infile: str | Path, outfile: str | Path | None = None) -> pd.DataFrame:
    """Convert one text file exported by TDX to CSV."""
...
async def covert(src: str | Path, dst: str | Path) -> pd.DataFrame:
    """Compatibility alias for the historical misspelled async entry point."""

    return await convert(src, dst)


def batch(src: str | Path, dst: str | Path) -> list[pd.DataFrame]:
    """Convert all TDX text exports in *src* into the *dst* directory."""
    ...
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix='mootdx-tdx2csv') as pool:
        return list(
            pool.map(
                lambda path: txt2csv(path, destination / path.with_suffix('.csv').name),
                files,
            )
        )
